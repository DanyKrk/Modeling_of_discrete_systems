...
public class Point {
	...
	private void addRepulsionFromOneself(){
		Iterator firstNeighboursIterator = this.neighbors.iterator();
		while (firstNeighboursIterator.hasNext()) {
			Point firstNeigh = (Point) firstNeighboursIterator.next();
			Iterator secondNeighboursIterator = firstNeigh.neighbors.iterator();
			while(secondNeighboursIterator.hasNext()){
				Point secondNeigh = (Point) secondNeighboursIterator.next();
				secondNeigh.setStaticField(secondNeigh.getStaticField() + 1);
			}
			firstNeigh.setStaticField(firstNeigh.getStaticField() + 1);
		}
	}

	private void removeRepulsionFromOneself(){
		Iterator firstNeighboursIterator = this.neighbors.iterator();
		while (firstNeighboursIterator.hasNext()) {
			Point firstNeigh = (Point) firstNeighboursIterator.next();
			Iterator secondNeighboursIterator = firstNeigh.neighbors.iterator();
	...
	public void becomePedestrian(){
		...
		this.blocked = true;
		this.addRepulsionFromOneself();
	}

	public void becomeFloor(){
		this.type = 0;
		this.isPedestrian = false;
		this.removeRepulsionFromOneself();
	}
